...
class KonusmaciTanimaSistemi:
    ...
    def ses_dosyasini_isle(self, ses_yolu, sample_rate=16000, max_duration=5):
        """Ses dosyasını önişlemeye tabi tutar"""
        try:
    ...
    def ses_tahmini(self, ses_dosyasi):
        ...
        try:
            ...
            if not ses_dosyasi.lower().endswith('.wav'):
                print("Hata: Sadece WAV formatı destekleniyor")
                return None
    ...
    def run(self):
        try:
            ...
            if self.is_folder:
                ...
                if not files:
                    self.error.emit("Klasörde WAV dosyası bulunamadı!")
                    return
...
class FinalModelTester(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Üst panel minimalist, alt panel daha büyük (1:3 oranı)
    ...
    def select_audio(self, is_folder=False):
        ...
        if is_folder:
            ...
            file_path, _ = QFileDialog.getOpenFileName(
                self, "Ses Dosyasını Seç", "", "WAV Dosyaları (*.wav)"
            )
            if file_path:
