...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                              ...
                              QHeaderView, QMessageBox, QSplitter, QComboBox, QFrame)
    ...
    def ses_tahmini(self, ses_dosyasi):
        ...
        try:
            ...
            return {
                ...
                'file_path': ses_dosyasi  # Ses dosyası yolunu da ekle
            }
...
class AudioPlayer:
    ...
    def get_status(self):
        ...
        if self.current_sound and self.is_playing:
            # Pygame mixer'da ses bitip bitmediğini kontrol et
            if not pygame.mixer.get_busy():
                self.is_playing = False
        return self.is_playing

#==================== TAHMİN THREAD SINIFI ====================
                ...
                for i, file in enumerate(files):
                    ...
                    if tahmin:
                        all_predictions.append({
                            ...
                            "file_path": tahmin['file_path']  # Ses dosyası yolunu ekle
                        })
                    # İlerlemeyi bildir
...
class FinalModelTester(QMainWindow):
    def __init__(self):
        ...
        self.dataset_path = "veriseti"  # Veri seti klasörü
        
        # UI ayarla
        self.setup_ui()
        
        # Timer for audio status check
    ...
    def setup_ui(self):
        ...
        # Sonuçlar Tablosu ve ses kontrolleri
        ...
        self.play_prediction_btn = QPushButton("Tahmin Sesi")
        self.play_prediction_btn.clicked.connect(self.play_prediction_audio)
        self.play_prediction_btn.setEnabled(False)
        self.play_prediction_btn.setFixedWidth(120)
        
        self.play_dataset_btn = QPushButton("Veri Seti Sesi")
        self.play_dataset_btn.clicked.connect(self.play_dataset_audio)
        self.play_dataset_btn.setEnabled(False)
        self.play_dataset_btn.setFixedWidth(120)
        
        self.stop_audio_btn = QPushButton("Durdur")
        ...
        audio_controls_layout.addWidget(self.play_prediction_btn)
        audio_controls_layout.addWidget(self.play_dataset_btn)
        ...
        results_layout.addLayout(audio_controls_layout)
        ...
        self.person_info_label.setAlignment(Qt.AlignCenter)
        self.person_info_label.setWordWrap(True)
        
        image_layout.addWidget(self.image_label)
        image_layout.addWidget(self.person_info_label)
    ...
    def on_table_item_clicked(self, item):
        ...
        try:
            ...
            # Ses butonlarını aktif et
            if self.current_predictions:
                self.play_prediction_btn.setEnabled(True)
                self.play_dataset_btn.setEnabled(True)
    ...
    def load_person_image(self, person_num):
        ...
        try:
            ...
            if os.path.exists(image_path):
                ...
                # Kişi bilgisini göster
                self.person_info_label.setText(f"Person {person_num}")
            else:
    ...
    def play_prediction_audio(self):
        """Tahmin edilen sesi çal"""
        ...
        if current_row < len(self.current_predictions):
            prediction = self.current_predictions[current_row]
            if 'file_path' in prediction:
                success = self.audio_player.play_audio(prediction['file_path'])
                if success:
                    self.statusBar().showMessage(f"Çalınıyor: {os.path.basename(prediction['file_path'])}")
                else:
                    self.statusBar().showMessage("Ses çalınamadı!")
    
    def play_dataset_audio(self):
        """Veri setinden doğruluk kontrolü için sesi çal"""
        if not self.current_predictions:
            return
        ...
        if current_row < len(self.current_predictions):
            prediction = self.current_predictions[current_row]
            person_label = prediction['tahmin_etiketi']
            
            # Veri setindeki ilgili kişinin sesini bul
            person_folder = os.path.join(self.dataset_path, person_label)
            
            if os.path.exists(person_folder):
                # Klasördeki ilk ses dosyasını bul
                supported_formats = ['.wav', '.mp3', '.m4a', '.flac', '.ogg', '.wma']
                for file in os.listdir(person_folder):
                    if any(file.lower().endswith(fmt) for fmt in supported_formats):
                        ...
                        if success:
                            self.statusBar().showMessage(f"Çalınıyor (Veri Seti): {file}")
                            return
                
                self.statusBar().showMessage("Veri setinde ses dosyası bulunamadı!")
            else:
                self.statusBar().showMessage(f"Veri seti klasörü bulunamadı: {person_folder}")
    
    def stop_audio(self):
        """Çalan sesi durdur"""
        self.audio_player.stop_audio()
        self.statusBar().showMessage("Ses durduruldu")
    
    def check_audio_status(self):
        """Ses durumunu kontrol et ve butonları güncelle"""
        if not self.audio_player.get_status():
            # Ses bittiyse durum çubuğunu güncelle
            if self.statusBar().currentMessage().startswith("Çalınıyor"):
                self.statusBar().showMessage("Ses tamamlandı", 2000)
    ...
    def update_json_output(self):
        ...
        # JSON için tahminleri sadece gerekli alanlarla hazırla
        json_predictions = []
        for pred in self.current_predictions:
            json_predictions.append({
                "id": pred["id"],
                "tahmin_etiketi": pred["tahmin_etiketi"]
            })
    ...
    def save_json(self):
        ...
        if file_path:
            try:
                # JSON verisi oluştur - ses dosyası yolları dahil değil
                json_predictions = []
                for pred in self.current_predictions:
                    json_predictions.append({
                        "id": pred["id"],
